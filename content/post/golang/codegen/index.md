...
```
可以看到`User`和`GetGetUserParams`唯一的差别只是它们的tag是不同的而已， 上面演示的只是Get请求解析参数的场景， 如果还有类似的其他场景呢， 比如使用`ShouldBindJson`, 那么我的tag里面就需要有json呢？  
我们希望我们有一个聪明的工具， 可以帮助我们基于`User`以及其他的结构体， 自动生成生成`GetUserParmas`之类的boilerplate codes.
...
### 理解结构体
理解结构体， 意味着我们首先需要知道结构体的名称， 结构体的字段， 字段类型， 字段标签, .etc， 譬如上面的`User`结构体中个字段信息。实现这个目标的方法大体上会有两种：
- 反射
- 将代码解析成抽象语法树然后遍历
...
接着我们会沿着ast.File这棵语法树进行便利， 找到我们需要的结构体对象：
...
```
collectStructs的核心其实是通过ast.Inspect进行遍历AST树， 它的第二个参数是一个用来遍历遍历节点且参数为ast.Node的回调函数， 回调函数如果返回true， 会继续遍历。  
上面的代码会遍历整个AST树， 然后将目标对象ast.StructType收集起来返回， 注意我们这里其实自定义了一个structType对象， 原因在于ast.StructType没有包含结构体名称的字段， 所以我们需要自定义一个（结构体名称后续是需要用到的）
...
```
其实就是用了递归下降的方法去遍历整棵树，通过对节点类型的判断， 使用不同的方法(Walk, walkDeclList等)来更深一步地进行递归调用。

我们收集完了所有结构体之后， 为了方便后续的代码生成环节， 我们这里对收集的结构体进行进一步的挖掘， 获取结构体中的字段名称、类型、Tag等信息：
```golang
...
```
代码整体比较简单， 就是遍历StructType的Fields.List来获取每个结构体字段的详细信息， 最后汇总成一个map[string]structComponets字典
